...
class LowonganKerjaTerdaftar extends CI_Controller {
    ...
    function Search()
    {
        ...
        if ($this->session->userdata('id'))
        {
            $config['total_rows'] = $this->DetailLKByProvinsiModel->totalDetailLKByProvinsiSearch($key);
            $config['base_url'] = base_url()."DirektoratPKK/LowonganKerjaTerdaftar/Index";
            $config['per_page'] = 10;
            $config['uri_segment'] = '4';
            ...
            $config['num_tag_close'] = '</li>';
            $this->pagination->initialize($config);
            $query = $this->DetailLKByProvinsiModel->getDetailLKByProvinsiSearch(10,$this->uri->segment(4),$key);
            $data['DetailLKByProvinsiModel'] = null;
            if($query){
                $data['DetailLKByProvinsiModel'] =  $query;
            }
