 ...
 * Time: 2:13
 */
class RekapFungsionalPusat extends CI_Controller
{
    ...
    function index($offset=0)
    {
        ...
        //print_r($page);
        $query = $this->rekapjabatanfungsionalpusatModel->getRekapFungsionalPusat(10,$this->uri->segment(4));
        //$query = $this->rekapjabatanfungsionalpusatModel->getDetailPKByProvinsi(5,$page);
        $data['rekapjabatanfungsionalpusatModel'] = null;
        if($query){
            $data['rekapjabatanfungsionalpusatModel'] =  $query;
        }
    ...
    function Search()
    {
        if ($this->session->userdata('id'))
        {
            ...
            $config['total_rows'] = $this->rekapjabatanfungsionalpusatModel->totalRekapFungsionalPusatSearch($key);
            $config['base_url'] = base_url()."DirektoratPTKDN/RekapFungsionalPusat/Index";
            $config['per_page'] = 10;
            $config['uri_segment'] = '4';
            ...
            //$page = $this->uri->segment(3);
            //print_r($page);
            $query = $this->rekapjabatanfungsionalpusatModel->getRekapFungsionalPusatSearch(10,$this->uri->segment(4), $key);
            //$query = $this->rekapjabatanfungsionalpusatModel->getDetailPKByProvinsi(5,$page);
            $data['rekapjabatanfungsionalpusatModel'] = null;
            if($query){
                $data['rekapjabatanfungsionalpusatModel'] =  $query;
    ...
    function UploadExcel(){
        ...
        $this->db->empty_table('rekapjabatanfungsionalpusat');
        ...
        for ($i = 3; $i<=$objPHPExcel->setActiveSheetIndex(0)->getHighestRow(); $i++) {
            ...
            $this->rekapjabatanfungsionalpusatModel->AddRekapFungsionalPusat($dataInput);
        }
        redirect('DirektoratPTKDN/RekapFungsionalPusat');
    }
