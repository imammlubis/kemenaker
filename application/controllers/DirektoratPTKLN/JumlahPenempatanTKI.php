 ...
 * Time: 2:38
 */
class JumlahPenempatanTKI extends CI_Controller
{
    ...
    function index($offset=0)
    {
        ...
        $query = $this->jumlahpenempatantkiModel->get(10,$this->uri->segment(4));
        $data['jumlahpenempatantkiModel'] = null;
        if($query){
            $data['jumlahpenempatantkiModel'] =  $query;
        }
    ...
    function Search()
    {
        if ($this->session->userdata('id'))
        {
            ...
            $config['total_rows'] = $this->jumlahpenempatantkiModel->totalSearch($key);
            $config['base_url'] = base_url()."DirektoratPTKLN/JumlahPenempatanTKI/Index";
            $config['per_page'] = 10;
            $config['uri_segment'] = '4';
            ...
            $config['num_tag_close'] = '</li>';
            $this->pagination->initialize($config);
            $query = $this->jumlahpenempatantkiModel->getSearch(10,$this->uri->segment(4), $key);
            $data['jumlahpenempatantkiModel'] = null;
            if($query){
                $data['jumlahpenempatantkiModel'] =  $query;
            }
    ...
    function UploadExcel(){
        ...
        $this->db->empty_table('jumlahpenempatantki');
        ...
        for ($i = 2; $i<=$objPHPExcel->setActiveSheetIndex(0)->getHighestRow(); $i++) {
            ...
            $jumlahTki = $objPHPExcel->getActiveSheet()->getCellByColumnAndRow(1, $i);
            ...
            $this->jumlahpenempatantkiModel->add($dataInput);
        }
        redirect('DirektoratPTKLN/JumlahPenempatanTKI');
    }
}