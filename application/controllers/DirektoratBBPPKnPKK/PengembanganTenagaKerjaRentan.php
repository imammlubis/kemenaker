 ...
 * Time: 17:19
 */
class PengembanganTenagaKerjaRentan extends CI_Controller
{
    ...
    function index($offset=0)
    {
        ...
        $query = $this->pengembangantenagakerjarentanModel->get(10,$this->uri->segment(4));
        $data['pengembangantenagakerjarentanModel'] = null;
        if($query){
            $data['pengembangantenagakerjarentanModel'] =  $query;
        }
    ...
    function UploadExcel(){
        ...
        $this->db->empty_table('pengembangantenagakerjarentan');
        ...
        for ($i = 2; $i<=$objPHPExcel->setActiveSheetIndex(0)->getHighestRow(); $i++) {
            ...
            $jumlahOutput = $objPHPExcel->getActiveSheet()->getCellByColumnAndRow(2, $i);
            $jumlahPPM = $objPHPExcel->getActiveSheet()->getCellByColumnAndRow(3, $i);
            ...
            $dataInput = array(
                ...
                'JUMLAHPENDAMPING' => $jumlahPPM,
                'TAHUN' => $tahun,
            );
            $this->pengembangantenagakerjarentanModel->add($dataInput);

        }
        redirect('DirektoratBBPPKnPKK/PengembanganTenagaKerjaRentan');
    }
